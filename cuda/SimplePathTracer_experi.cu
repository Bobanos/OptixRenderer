...
__device__ float3 getAlbedo(const HitGroupDataLambert* sbt)
{
    ...
    return sbt->albedo * b0
        + sbt->albedo * bary.x
        + sbt->albedo * bary.y;
}

template <typename MaterialData>
__device__ float3 getEmission(const MaterialData* sbt)
{
    // If we have an emissive texture, sample it
    if (sbt->emission_texture != 0) {
        ...
        //v = 1.f - v;  // flip V to match CUDA's top-left origin

        float4 t = tex2D<float4>(sbt->emission_texture, u, v);
        if (nonZero(sbt->emission)) {
            return make_float3(t.x, t.y, t.z) * sbt->emission;
        }
        return make_float3(t.x, t.y, t.z);
    }

    // Fallback: Use the flat material color stored in the SBT record
    return sbt->emission;
}
...
extern "C" __global__ void __miss__ms() {
    ...
    if (params.envmap.has_envmap) {
        float3 envColor = sampleEnvmap(params.envmap.texture, ray_dir);
        // Apply exposure: exposure = log2 scale
        envColor = envColor * params.envmap.scale * powf(2.0f, params.envmap.exposure);
...
extern "C" __global__ void __closesthit__ch()
{
    ...
    float3 emissive_radiance = getEmission(sbt) * params.light_intensity; // Allow bright emissive glass
    float3 final_color = emissive_radiance;
    ...
    if (cont && depth >= 2) {
        ...
        if (random_float(seed) > rr_prob) cont = false;
    ...
    if (cont) {
        ...
        if (random_float(seed) < prob_diff) {
            // Sample diffuse lobe: cosine-weighted hemisphere
            wi_bounce       = sampleCosineHemisphere(normal, random_float(seed), random_float(seed));
            ...
            float ps;
            wi_bounce       = sampleBlinnPhong(wo, normal, shininess, random_float(seed), random_float(seed), ps);
            float pd        = pdfCosine(wi_bounce, normal);
            pdf_bounce      = prob_diff * pd + (1.f - prob_diff) * ps;  // mixture PDF
        }
    ...
    //final_color = indirect * albedo;  // modulate indirect by albedo for energy conservation
    final_color = final_color + indirect * albedo;
...
// ------------------------------------------------------------------
// Closest hit: glass/refractive with Blinn-Phong highlights
// ------------------------------------------------------------------
extern "C" __global__ void __closesthit__glass()
{
    ...
    // Get vertex color for glass tint
    const int   prim_idx = optixGetPrimitiveIndex();
    const uint3 tri      = sbt->indices[prim_idx];
    const float2 bary    = optixGetTriangleBarycentrics();
    const float  b0      = 1.0f - bary.x - bary.y;
    
    float3 glass_color = sbt->albedo * b0
                       + sbt->albedo * bary.x
                       + sbt->albedo * bary.y;
    ...
    unsigned int seed = params.random_seed + (current_depth * 12345) + __float_as_uint(hit_point.x) ^ __float_as_uint(hit_point.y);

    // Emission contribution
    //float3 emissive_radiance = glass_color * 15;//sbt->emissive_intensity;
	float3 emissive_radiance = getEmission(sbt) * params.light_intensity; // Allow bright emissive glass
    float3 final_color = emissive_radiance;

    // Indirect lighting - refraction/reflection
    if (current_depth < params.max_bounce_depth) {
        float eta = entering ? (1.0f / ior) : ior;
        float cos_theta = fminf(fabs(dot(ray_direction, world_normal)), 1.0f);
        ...
        if (rand_val < fresnel || eta * sin_theta > 1.0f) {
            // REFLECTION
            ...
            unsigned int p4 = __float_as_uint(throughput.x);
            unsigned int p5 = __float_as_uint(throughput.y);
            unsigned int p6 = __float_as_uint(throughput.z);
            ...
            float3 indirect = make_float3(__uint_as_float(p0),
                                __uint_as_float(p1),
                                __uint_as_float(p2));
            
            final_color = final_color + indirect / fresnel;
        }
        else {
            ...
            final_color = final_color + indirect * glass_color / (1.0f - fresnel);
        }
    }

    final_color = clamp(final_color, 0.0f, 10.0f);
