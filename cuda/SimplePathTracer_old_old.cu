...
extern "C" __global__ void __miss__ms() {
    ...
    if (params.envmap.has_envmap) {
        ...
        Le = make_float3(0.0f, 0.0f, 0.0f); // black
    }
...
// ------------------------------------------------------------------
// Blinn-Phong BRDF helper
// ------------------------------------------------------------------
__device__ float3 blinn_phong_brdf(
    ...
    float shininess)
{
    ...
    if (ndotl > 0.0f) {
        // Diffuse component
        brdf = brdf + diffuse_color * (1.0f / M_PI) * ndotl;
        
        // Specular component (Blinn-Phong)
        float3 half_vec = normalize(view_dir + light_dir);
        float ndoth = fmaxf(0.0f, dot(normal, half_vec));
        float spec = powf(ndoth, shininess);
        
        // Fresnel approximation
        float f0 = 0.04f;
        float fresnel = f0 + (1.0f - f0) * powf(1.0f - fmaxf(0.0f, dot(half_vec, view_dir)), 5.0f);
        
        brdf = brdf + specular_color * fresnel * spec;
    }
...
extern "C" __global__ void __closesthit__glass()
{
    ...
    float3 glass_color = sbt->vertices[tri.x].color * b0
                       + sbt->vertices[tri.y].color * bary.x
                       + sbt->vertices[tri.z].color * bary.y;
    ...
    float3 view_dir = normalize(-ray_direction);
    ...
    for (int light_idx = 0; light_idx < params.num_lights; light_idx++) {
        ...
        if (shadow > 0.0f) {
            float3 brdf = blinn_phong_brdf(view_dir, light_dir, world_normal,
                                          glass_color * 0.1f, // Low diffuse for glass
                                          make_float3(1.0f, 1.0f, 1.0f), // White specular
                                          64.0f); // High shininess for glossy glass
            final_color = final_color + light.color * brdf * atten * shadow;
        }
    }
